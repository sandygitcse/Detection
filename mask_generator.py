...
path_dir = f"Results/{args.dataset_name}/"
...
for folder in dirs:
    ...
    labels = labels[cb:cb+l*h].reshape(-1,h)
    ...
    if "50" in path:    #####################change it back to 25   ##########
        final_25 = mask 
    else:
        final_50 = mask
...
subprocess.run(["scp",file_p, f"sandy@cat.cse.iitb.ac.in:/mnt/cat/data/sandy/Forecasting/data/{args.dataset_name}/{args.dataset_name}_mask_roc.npy"])
print(f"File saved as sandy@cat.cse.iitb.ac.in:/mnt/cat/data/sandy/Forecasting/data/{args.dataset_name}/{args.dataset_name}_mask_roc.npy !!")
